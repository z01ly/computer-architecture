...
void v_add_optimized_chunks(double* x, double* y, double* z) {
  ...
  #pragma omp parallel
  {
      ...
      if (ARRAY_SIZE % num_threads != 0) {
          chunk_size = (ARRAY_SIZE + num_threads) / num_threads;
          last_chunk_size = ARRAY_SIZE % num_threads; 
      } else {
      ...
      if (thread_id != (num_threads - 1)) {
          ...
          for (int i = thread_id * chunk_size; i < ARRAY_SIZE; i++)
              z[i] = x[i] + y[i];
      }
...
double dotp_manual_optimized(double* x, double* y, int arr_size) {
  ...
  // for (int i = 0; i < num_threads; i++) sum_arr[i] = 0.0;
  #pragma omp parallel 
  {
      // int thread_id = omp_get_thread_num();
      double sum_arr = 0.0;
#pragma omp for
      for (int i = 0; i < arr_size; i++)
          // sum_arr[thread_id] += x[i] * y[i];
          sum_arr += x[i] * y[i];
#pragma omp critical
      // global_sum += sum_arr[thread_id];
      global_sum += sum_arr;
  }
  return global_sum;
}
